 ...
 * @param initial_state 23-bit initial state
 * @param output Output buffer for PRN chips (-1 or +1)
 * @param num_chips Number of chips to generate
 *
 * T.018 compliant: Fibonacci LFSR, RIGHT shift, LSB output
 * Output: X0 (LSB), Logic 1 → -1, Logic 0 → +1
 * Feedback: X0 ⊕ X18
 */
void dsss_generate_prn(uint32_t initial_state, int8_t *output, size_t num_chips) {
    ...
    for (size_t i = 0; i < num_chips; i++) {
        ...
        // LFSR feedback: X0 ⊕ X18 (T.018 Appendix D)
...
void dsss_generate_prn_sequences(int8_t *prn_i, int8_t *prn_q) {
    // COSPAS-SARSAT Normal mode initial states (T.018 Table 2.2)
    // I channel: [0000000000000000000000 1] = 0x000001
    uint32_t init_i = 0x000001;

    // Q channel: [00110101100000111111100] = 0x1AC1FC
    // First 64 chips: 3F83 58BA D030 F231
    uint32_t init_q = 0x1AC1FC;

    // Generate full sequences (150 bits * 256 = 38400 chips each)
...
phase_found:
    ...
    // Extract I and Q chip streams from demodulated bits
    // rx_sig structure: [I0, Q0, I1, Q1, I2, Q2, ...]
    // p_best indicates symbol offset (0 or 1)
    ...
    for (size_t i = 0; i < DSSS_PACKET_CHIPS && (i + p_best) < num_symbols; i++) {
        rx_ci[i] = rx_sig[2 * (i + p_best)];      // I channel
        rx_cq[i] = rx_sig[2 * (i + p_best) + 1];  // Q channel
    }

    // DEBUG: Dump received chips for comparison
    FILE *chip_dump = fopen("chips_received.bin", "wb");
    if (chip_dump) {
        for (size_t i = 0; i < DSSS_PACKET_CHIPS; i++) {
            // Convert 0/1 to +1/-1 format to match reference
            int8_t chip_i = (rx_ci[i] == 0) ? 1 : -1;
            int8_t chip_q = (rx_cq[i] == 0) ? 1 : -1;
            fwrite(&chip_i, sizeof(int8_t), 1, chip_dump);
            fwrite(&chip_q, sizeof(int8_t), 1, chip_dump);
        }
        fclose(chip_dump);
    }
    ...
    // Convert received chips to +1/-1 format for correlation
    // QPSK demod gives: re>0 → 0, re<0 → 1
    // Transmitted +1 chip → demod 0, transmitted -1 chip → demod 1
    // So: rx=0 → -1, rx=1 → +1 for correlation with PRN
    ...
    // Correlate with PRN sequences (multiply, not XOR)
    // Transmitted: bit=0 → +prn, bit=1 → -prn
    // After correlation: bit=0 → positive sum, bit=1 → negative sum
    ...
    for (int bit_idx = 0; bit_idx < DSSS_PACKET_BITS / 2; bit_idx++) {
        ...
        // Positive correlation → bit=0, negative correlation → bit=1
