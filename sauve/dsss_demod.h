...
/**
 * @brief Resolve phase ambiguity by despreading preamble
 *
 * Tests all phase/swap combinations by:
 * 1. Applying phase rotation (0°, 90°, 180°, 270°)
 * 2. Demodulating to bits (QPSK → I/Q bitstreams)
 * 3. Despreading preamble with PRN sequences
 * 4. Correlating with known preamble pattern (all zeros)
 * 5. Selecting combination with highest correlation
 *
 * This approach tests phase ambiguity AFTER despreading, which provides
 * clear discrimination between correct and incorrect configurations.
 *
 * @param qpsk_symbols  Input QPSK symbols at chip rate (38400)
 * @param num_symbols   Number of symbols
 * @param prn_i         PRN sequence for I channel (38400 chips)
 * @param prn_q         PRN sequence for Q channel (38400 chips)
 * @param best_rotation Output: optimal phase rotation (0-3)
 * @param best_swap     Output: optimal I/Q swap (0=normal, 1=swapped)
 * @return Correlation metric of best configuration (0.0-1.0)
 */
