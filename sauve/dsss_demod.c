...
int dsss_timing_recovery_oqpsk(const float complex *oqpsk_in,
    ...
    // Create QPSK-equivalent signal: I(t) + j*Q(t) with Q delayed back by Tc/2
    // Since input OQPSK has Q already delayed, we delay Q further (not advance)
    for (size_t i = 0; i < num_samples; i++) {
        float i_val = crealf(oqpsk_in[i]);
        float q_val = (i >= (size_t)q_delay_samples) ?
                      cimagf(oqpsk_in[i - q_delay_samples]) : 0.0f;
        qpsk_equiv[i] = i_val + I * q_val;
    }

    printf("[TIMING OQPSK] Q channel delayed by %d samples (Tc/2)\n", q_delay_samples);
...
// =============================================================================
// PHASE AMBIGUITY RESOLUTION BY DESPREADING
// =============================================================================

/**
 * @brief Resolve phase ambiguity by despreading and correlating preamble
 *
 * Tests all 8 combinations (4 rotations × 2 swaps) by despreading the preamble
 * and correlating with the known all-zero pattern. Returns the configuration
 * with the highest correlation.
 *
 * Algorithm for each combination:
 * 1. Apply phase rotation: symbols * exp(j*k*π/2) where k=0,1,2,3
 * 2. Demodulate QPSK to bits (decision on real/imag > 0)
 * 3. Extract I and Q bitstreams (odd/even bits, with optional swap)
 * 4. XOR with PRN sequences to despread
 * 5. Correlate despread preamble with known pattern (all zeros)
 * 6. Compute correlation metric
 *
 * @return Correlation metric of best configuration (0.0 to 1.0)
 */
float dsss_resolve_phase_by_despread(const float complex *qpsk_symbols,
                                     size_t num_symbols,
                                     const int8_t *prn_i,
                                     const int8_t *prn_q,
                                     int *best_rotation,
    ...
    printf("[PHASE DESPREAD] Starting phase ambiguity resolution\n");
    printf("[PHASE DESPREAD] Testing 8 combinations (4 rotations × 2 swaps)\n");

    // Test preamble (first 6400 chips = 25 bits × 256 chips/bit per channel)
    const int preamble_bits = 25;  // 25 bits I + 25 bits Q
    const int chips_per_bit = 256;
    const int test_chips = preamble_bits * chips_per_bit;  // 6400 chips per channel

    if ((size_t)test_chips * 2 > num_symbols) {
        fprintf(stderr, "[PHASE DESPREAD] ERROR: Not enough symbols (%zu < %d)\n",
                num_symbols, test_chips * 2);
        *best_rotation = 0;
        *best_swap = 0;
        return 0.0f;
    }
    ...
    *best_swap = 0;
    ...
    // Phase rotations: 0°, 90°, 180°, 270°
    const float complex rotations[4] = {
        1.0f + 0.0f*I,      // 0° = exp(j*0)
        0.0f + 1.0f*I,      // 90° = exp(j*π/2)
        -1.0f + 0.0f*I,     // 180° = exp(j*π)
        0.0f - 1.0f*I       // 270° = exp(j*3π/2)
    };

    // Allocate work buffers
    uint8_t *bits_i = malloc(test_chips);
    uint8_t *bits_q = malloc(test_chips);
    uint8_t *despread_i = malloc(test_chips);
    uint8_t *despread_q = malloc(test_chips);

    if (!bits_i || !bits_q || !despread_i || !despread_q) {
        fprintf(stderr, "[PHASE DESPREAD] ERROR: Memory allocation failed\n");
        free(bits_i); free(bits_q); free(despread_i); free(despread_q);
        return 0.0f;
    }
    ...
    for (int swap = 0; swap <= 1; swap++) {
        for (int rot = 0; rot < 4; rot++) {
            ...
            // Step 1: Apply phase rotation
            float complex rotation = rotations[rot];

            // Step 2: Demodulate QPSK symbols to chips
            // Each QPSK symbol contains both I and Q chip values
            // QPSK: real part = I chip, imag part = Q chip
            for (int i = 0; i < test_chips; i++) {
                float complex rotated = qpsk_symbols[i] * rotation;

                // Extract I and Q chip values
                // Positive → bit 1, negative → bit 0
                uint8_t chip_i = (crealf(rotated) > 0.0f) ? 1 : 0;
                uint8_t chip_q = (cimagf(rotated) > 0.0f) ? 1 : 0;
            ...
            // Step 3: Despread with PRN (XOR)
            for (int i = 0; i < test_chips; i++) {
                // PRN: -1 → bit 0, +1 → bit 1
                uint8_t prn_bit_i = (prn_i[i] > 0) ? 1 : 0;
                uint8_t prn_bit_q = (prn_q[i] > 0) ? 1 : 0;

                despread_i[i] = bits_i[i] ^ prn_bit_i;
                despread_q[i] = bits_q[i] ^ prn_bit_q;
            }

            // Step 4: ML decoding (majority vote) + correlation with known preamble
            // Group 256 chips into 1 bit, count zeros, if >128 → bit=0, else bit=1
            int matches_i = 0, matches_q = 0;
            for (int bit = 0; bit < preamble_bits; bit++) {
                int sum_i = 0, sum_q = 0;
                for (int chip = 0; chip < chips_per_bit; chip++) {
                    int idx = bit * chips_per_bit + chip;
                    if (despread_i[idx] == 0) sum_i++;
                    if (despread_q[idx] == 0) sum_q++;
                }
                // Majority vote: if more than half chips are 0, decoded bit is 0
                uint8_t decoded_bit_i = (sum_i > chips_per_bit / 2) ? 0 : 1;
                uint8_t decoded_bit_q = (sum_q > chips_per_bit / 2) ? 0 : 1;

                // Preamble bits are all 0
                if (decoded_bit_i == 0) matches_i++;
                if (decoded_bit_q == 0) matches_q++;
            }

            float corr = (float)(matches_i + matches_q) / (float)(preamble_bits * 2);

            printf("[PHASE DESPREAD]   swap=%d rot=%d (phase=%3d°): corr=%.3f (I=%.3f Q=%.3f)\n",
                   swap, rot, rot * 90,
                   corr,
                   (float)matches_i / test_chips,
                   (float)matches_q / test_chips);

            // Debug: print first 20 despread chips for swap=0 rot=0
            if (swap == 0 && rot == 0) {
                printf("[PHASE DESPREAD]     First 20 despread I: ");
                for (int i = 0; i < 20; i++) printf("%d", despread_i[i]);
                printf("\n");
                printf("[PHASE DESPREAD]     First 20 despread Q: ");
                for (int i = 0; i < 20; i++) printf("%d", despread_q[i]);
                printf("\n");
                printf("[PHASE DESPREAD]     First 20 chips I: ");
                for (int i = 0; i < 20; i++) printf("%d", bits_i[i]);
                printf("\n");
                printf("[PHASE DESPREAD]     First 20 PRN I: ");
                for (int i = 0; i < 20; i++) printf("%d", (prn_i[i] > 0) ? 1 : 0);
                printf("\n");
            }
    ...
    printf("[PHASE DESPREAD] Best configuration: swap=%d rot=%d (phase=%d°) corr=%.3f\n",
           *best_swap, *best_rotation, (*best_rotation) * 90, best_corr);

    if (best_corr < 0.7f) {
        fprintf(stderr, "[PHASE DESPREAD] WARNING: Low correlation (%.3f < 0.7)\n", best_corr);
    }
...
// =============================================================================
// PHASE AMBIGUITY RESOLUTION (OLD)
// =============================================================================
