...
- **DSSS OQPSK (2G)** — `dsss_demod.c` : chaîne de traitement linéaire (sans
  ...
  ±16 kHz, ce qui englobe les erreurs de centroïde observées lors du balayage
  sur des salves SGB réelles du CNES.
...
proprement le BCH. La recherche d'acquisition couvre désormais ±16 kHz afin
d'absorber les erreurs de centroïde observées sur des rafales SGB CNES réelles.
Les essais du relais firmin et les validations locales RTL/Yagi donnent des
taux élevés, avec FGB restant dans sa plage habituelle. Ces valeurs doivent
être traitées comme des contrôles terrain propres à chaque passe, pas comme des
taux globaux fixes.
