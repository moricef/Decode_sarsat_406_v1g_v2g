...
/**
 * @brief Initialize AGC with specified parameters (MATLAB-compatible)
 */
static void agc_init(agc_t *agc, int sps) {
    agc->avg_power = 0.0f;
    agc->sps = sps;
}

/**
 * @brief Process samples through AGC (MATLAB simpleAGC algorithm)
 *
 * MATLAB algorithm:
 * - First sample: avgPower = magnitude²
 * - Update avgPower every (10*sps) samples: avgPower = 0.9*avgPower + 0.1*power
 * - Apply gain to ALL samples: gain = 1.0 / sqrt(avgPower)
 */
static void agc_process(agc_t *agc, const float complex *input,
                       float complex *output, size_t length) {
    int update_interval = 10 * agc->sps;  // MATLAB: 10.0 * sps

    for (size_t i = 0; i < length; i++) {
        // Update average power
        if (i == 0) {
            // MATLAB: First sample
            float mag = cabsf(input[0]);
            agc->avg_power = mag * mag;
        } else {
            // MATLAB: Update every (10*sps) samples
            // Check if (i+1) % update_interval == 1
            if (((i + 1) % update_interval) == 1) {
                float mag = cabsf(input[i]);
                float inst_power = mag * mag;
                agc->avg_power = 0.9f * agc->avg_power + 0.1f * inst_power;
            }
        }

        // Apply gain to current sample
        // MATLAB: if avgPower > 0 then gain = 1/sqrt(avgPower) else gain = 1
        float gain;
        if (agc->avg_power > 0.0f) {
            gain = 1.0f / sqrtf(agc->avg_power);
        } else {
            gain = 1.0f;
        }
...
static void carrier_sync_process(carrier_sync_t *sync, const float complex *input,
                                float complex *output, size_t length,
                                float *phase_error) {
    int delay = sync->sps / 2;  // OQPSK delay: sps/2 samples
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        // Phase error detector for OQPSK (MATLAB Algorithm, page 24)
        // en = sgn(Re{xn-sps/2}) × Im{xn-sps/2} - sgn(Im{xn}) × Re{xn}
        // Use every sps-th sample for symbol-rate processing
        if (i % sync->sps == 0 && (int)i >= delay) {
            // Get delayed sample xn-sps/2
            float re_delayed = crealf(output[i - delay]);
            float im_delayed = cimagf(output[i - delay]);

            // Get current sample xn
            float re_current = crealf(output[i]);
            float im_current = cimagf(output[i]);

            // OQPSK phase error detector
            // en = sgn(Re{xn-sps/2}) × Im{xn-sps/2} - sgn(Im{xn}) × Re{xn}
            float sgn_re_delayed = (re_delayed > 0) ? 1.0f : -1.0f;
            float sgn_im_current = (im_current > 0) ? 1.0f : -1.0f;

            float error = sgn_re_delayed * im_delayed - sgn_im_current * re_current;
...
int dsss_receive_burst(const float complex *ota_buffer,
    ...
    for (size_t n = 0; n < num_buffers; n++) {
        ...
        // Configure and process AGC (MATLAB-compatible)
        agc_t agc;
        agc_init(&agc, sps);
        ...
        if (n == 0) {
            ...
            float first_mag = cabsf(rx_agc_samples[0]);
            printf("AGC: avg_power=%.6f, first_sample=%.3f+j%.3f (mag=%.3f)\n",
                   power,
                   crealf(rx_agc_samples[0]), cimagf(rx_agc_samples[0]), first_mag);
        }
...
preamble_found:
    ...
    printf("[DEBUG] rxBurst (OQPSK): [0]=%.3f+j%.3f, [100]=%.3f+j%.3f\n",
           crealf(rx_burst[0]), cimagf(rx_burst[0]),
           crealf(rx_burst[100]), cimagf(rx_burst[100]));

    // Step 1 - Coarse Frequency Offset Estimation and Correction ON OQPSK
    // MATLAB: coarse = comm.CoarseFrequencyCompensator(Modulation="OQPSK", ...)
    // For OQPSK: Estimate using temporary QPSK conversion, apply correction to OQPSK
    printf("[DEBUG] Step 1: Coarse frequency compensation (MATLAB Modulation=OQPSK)\n");

    // Temporary OQPSK→QPSK conversion for frequency estimation only
    size_t qpsk_length = burst_length - sps / 2;
    float complex *temp_qpsk = malloc(burst_length * sizeof(float complex));
    for (size_t i = 0; i < qpsk_length; i++) {
        temp_qpsk[i] = crealf(rx_burst[i]) + I * cimagf(rx_burst[i + sps / 2]);
    }
    for (size_t i = qpsk_length; i < burst_length; i++) {
        temp_qpsk[i] = 0.0f;
    }

    // Estimate frequency offset using 4th-power on temporary QPSK
    float coarse_offset = estimate_coarse_frequency_offset(temp_qpsk, burst_length, fs);
    printf("Estimated coarse frequency offset = %.3f kHz\n", coarse_offset / 1000.0f);
    free(temp_qpsk);

    // Apply correction to OQPSK signal (phase rotation works on both OQPSK and QPSK)
    ...
    apply_frequency_offset(coarse_sync_out, burst_length, fs, coarse_offset);
    free(rx_burst);

    // Step 2 - Fine Frequency/Phase Correction (Carrier Synchronizer) ON OQPSK
    // MATLAB: carrierSync = comm.CarrierSynchronizer(Modulation="OQPSK", ...)
    printf("[DEBUG] Step 2: Carrier synchronization (MATLAB Modulation=OQPSK, loop_bw=0.01, damping=0.707)\n");
    ...
    printf("[DEBUG] carrierSyncOut (OQPSK): [0]=%.3f+j%.3f, [100]=%.3f+j%.3f\n",
           crealf(carrier_sync_out[0]), cimagf(carrier_sync_out[0]),
           crealf(carrier_sync_out[100]), cimagf(carrier_sync_out[100]));

    // Step 3 - Convert OQPSK to QPSK for preamble detection
    // MATLAB line 156: carrierSyncOutQPSK = [real(carrierSyncOut(1:end-sps/2)) + 1i*imag(carrierSyncOut(sps/2+1:end)); zeros(sps/2,1)];
    printf("[DEBUG] Step 3: Converting OQPSK to QPSK for preamble detection\n");
    float complex *carrier_sync_out_qpsk = malloc(burst_length * sizeof(float complex));
    for (size_t i = 0; i < qpsk_length; i++) {
        carrier_sync_out_qpsk[i] = crealf(carrier_sync_out[i]) + I * cimagf(carrier_sync_out[i + sps / 2]);
    }
    for (size_t i = qpsk_length; i < burst_length; i++) {
    ...
    // Step 4 - Timing Recovery ON OQPSK signal
    // MATLAB line 177: [syncedQPSK,timingError] = symbolSynchronizer(carrierSyncOut(FSPSampIdx:end));
    // CRITICAL: SymbolSynchronizer receives OQPSK (carrierSyncOut), NOT QPSK!
    // Parameters (SPS=8): NormalizedLoopBandwidth=0.001, DetectorGain=4, DampingFactor=2
    printf("[DEBUG] Step 4: Timing recovery on OQPSK (MATLAB SymbolSynchronizer Modulation=OQPSK)\n");
    ...
    // Process from FSP index to end ON OQPSK SIGNAL (carrier_sync_out, NOT carrier_sync_out_qpsk)
...
phase_found:
    ...
    free(prn_q);
    free(corr_buffer);
    free(carrier_sync_out);  // OQPSK signal after freq/phase corrections
    free(carrier_sync_out_qpsk);  // QPSK converted for preamble detection
    free(phase_err);
    free(synced_qpsk);
    free(timing_error);
    free(rx_sig);
