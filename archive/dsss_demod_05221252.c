...
static void dump_complex(const char *path, const float complex *p, size_t n) {
  ...
  if (f) {
    fwrite(p, sizeof(float complex), n, f);
    fclose(f);
  }
}
#else
...
int dsss_receive_burst(const float complex *ota_buffer, size_t buffer_length,
                       float sps, float fs, int max_doppler,
  ...
  size_t chip_buf =
      (size_t)((float)N / sps) + DESPREAD_SYNC_RANGE + DESPREAD_CHIPS_PER_BIT;
 ...
 * --------------------------------------------------------------- */
#define COARSE_FFT_MIN_CONF                                                    \
  2000.0f /* below this the 4th-power FFT is                                   \
             unreliable — use FFT-corr */
#define COARSE_FFT_MIN_HZ 1.0f
#define COARSE_FFT_MAX_HZ                                                      \
  25000.0f /* Pluto 40 ppm + RTL-SDR 1 ppm ≈ 16.5 kHz                        \
            */
  float coarse_freq_hz = 0.0f;
  {
    ...
    int acq_ok = 0;
    if (freq_acq_coarse_fft(post_rrc, (int)N, fs, chip_rate, &acq) == 0 &&
    ...
    if (!acq_ok) {
      ...
      if (n_chips_test > 12000)
        n_chips_test = 12000;
      float complex *chip_test =
          (float complex *)calloc(n_chips_test, sizeof(float complex));
      if (chip_test) {
        /* Build chip_test by a boxcar sum over each chip's samples,
         * not single-sample decimation: the chip timing phase is
         * unknown (burst_start is not chip-aligned) and a fixed
         * decimation phase samples the half-sine chips off-peak —
         * catastrophic at a half-chip error. The boxcar integrates
         * the whole chip → at most ~-6 dB of phase loss, no null;
         * adjacent-chip crosstalk averages out in the correlation. */
        ...
        if (freq_acq_fft_corr(chip_test, (int)n_chips_test, chip_rate, -8000.0f,
                              8000.0f, &acq2) == 0 &&
    ...
    //}
        ...
        /* ---------------------------------------------------------------
         * NOUVEAU : Stopper la rotation de la porteuse (Baseband) 
         * AVANT de séparer les voies Réelles et Imaginaires !
         * --------------------------------------------------------------- */
        ...
        for (size_t t = 0; t < N; t++) {
            float re = __real__ post_rrc[t];
            float im = __imag__ post_rrc[t];
            
            // Rotation complexe
            __real__ post_rrc[t] = re * ph_r - im * ph_i;
            __imag__ post_rrc[t] = re * ph_i + im * ph_r;
            
            // Mise à jour du phaseur
            float nr = ph_r * step_r - ph_i * step_i;
            float ni = ph_r * step_i + ph_i * step_r;
            
            // Normalisation périodique pour éviter la dérive float
            if ((t & 1023) == 0) {
        ...
        /* OQPSK delay : Maintenant on peut décaler I et Q sans "crosstalk" */
        int oqpsk_delay = (int)(sps / 2.0f + 0.5f);
        for (size_t t = 0; t < N; t++) {
            float ir = __real__ post_rrc[t];
            float qi = (t + (size_t)oqpsk_delay < N)
  ...
  n_chips = 0;
  {
    ...
    float init_code = 0.0f; /* scan disabled — hurts more than helps */

    if (tracking_init(&trk, fs, sps, 0.0f, init_code) != 0) {
      fprintf(stderr, "[dsss_demod] tracking_init failed\n");
      goto cleanup;
    }

    size_t n_chips_tracked = 0;
    int trk_rc = tracking_run(&trk, post_rrc, N, post_dec, &n_chips_tracked);
    tracking_free(&trk);
  ...
  /* DEBUG: dump tracking output chips (first window only) */
  {
    ...
    if (!once) {
      ...
      if (f) {
        fwrite(post_dec, sizeof(float complex), n_chips, f);
        fclose(f);
      }
    }
  }
